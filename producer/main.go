...
func produceRecord(provider *producerProvider) {
	...
	for i := int64(0); i < recordsNumber; i++ {
		...
		producer.Input() <- &sarama.ProducerMessage{Topic: topic, Key: nil, Value: sarama.StringEncoder(msgBytes)}
		time.Sleep(500 * time.Millisecond)
		//log.Printf("Producer: produced message- [%v]", msgToProduce)
	}
